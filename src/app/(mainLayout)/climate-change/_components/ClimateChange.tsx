...
const ClimateChange: React.FC<CovidProps> = ({ whatWedoData, language }) => {
    const covidFilterData = whatWedoData.filter((edu) => edu.category === 'Climate Change')
                ...
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ">
                    {covidFilterData?.map((data, index) => (
                        ...
                        >
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-green-700 text-[#fff] transition-transform transform translate-y-full group-hover:translate-y-0 duration-500 ease-in-out h-[250px] md:h-[300px] lg:h-[300px] md:rounded-t-3xl">
                                <div className="w-full p-2 md:p-4 lg:p-6 lg:h-full">
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-3">
                                            <h2 className="md:text-xl ">{language == 'ENG' ? data.english_title : data.bangla_title}</h2>
                                        </div>
