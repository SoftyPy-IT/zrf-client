...
const HealthServices: React.FC<CovidProps> = ({ healthServicesData, language }) => {
    const [visibleCount, setVisibleCount] = useState(6);
    const loadMore = () => {
        setVisibleCount((prevCount) => prevCount + 6);
    };

    const sortedHealtherviceData = healthServicesData?.sort((a: TWhatWeDo, b: TWhatWeDo) => {
                ...
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ">
                    {sortedHealtherviceData?.slice(0, visibleCount).map((data, index) => (
                        ...
                        >
                            ...
                            {/* Hover content */}
                            <div className="absolute inset-x-0 bottom-0 bg-green-700 text-[#fff] transition-transform transform translate-y-full group-hover:translate-y-0 duration-500 ease-in-out h-[230px] md:h-[230px] md:rounded-t-3xl">
